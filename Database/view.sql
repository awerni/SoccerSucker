...
SELECT p.username, name, firstname, nationality, 
       ...
       FROM player p
       LEFT OUTER JOIN (SELECT username, sum(points) AS grouppoints, count(*) AS groupgames, count(points) AS evalgroupgames FROM tipgame WHERE NOT kogame GROUP BY username) AS t1 ON p.username = t1.username
